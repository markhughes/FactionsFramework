...
public class Events_0_2_2 extends EventsLayer {
    ...
    public void onEventFactionsCreate(com.massivecraft.factions.event.FactionCreateEvent event) {
        EventFactionsCreate eventFactionsCreate = new EventFactionsCreate(event.getFaction().getTag(), FPlayers.getById(event.getFPlayer().getId()));
    ...
    public void onEventFactionsChunksChange(LandClaimEvent event) {
        ...
        if (eventFactionsChunksChange.getChunks().size() == 0) event.setCancelled(true);
            // If there are more than 1 or the only chunk there is not the existing chunk
        else if (eventFactionsChunksChange.getChunks().size() > 1 || this.getFinalChunk(eventFactionsChunksChange.getChunks()) != chunk) {
    ...
    public void onEventFactionsChunksChange(LandUnclaimAllEvent event) {
        ...
        for (FLocation flocation : flocations) {
            chunks.add(flocation.getWorld().getChunkAt(Math.toIntExact(flocation.getX()), Math.toIntExact(flocation.getZ())));
        ...
        if (eventFactionsChunksChange.getChunks().size() != chunks.size())
            ...
            doChange = true;
        else for (Chunk chunk : eventFactionsChunksChange.getChunks()) {
        ...
        handleNewUnclaims(event.getfPlayer(), event.getFaction(), chunks);
    ...
    public void onEventFactionsLeave(FPlayerLeaveEvent event) {
        ...
        else if (event.getReason() == PlayerLeaveReason.DISBAND) {
    ...
    private Boolean mimicUnclaim(com.massivecraft.factions.FPlayer fplayer, com.massivecraft.factions.Faction faction, FLocation flocation) {
        if (Econ.shouldBeUsed()) {
            ...
            if (Conf.bankEnabled && Conf.bankFactionPaysLandCosts)
                if (!Econ.modifyMoney(faction, refund, "to unclaim this land", "for unclaiming this land"))
                    return false;
                else if (!Econ.modifyMoney(fplayer, refund, "to unclaim this land", "for unclaiming this land"))
                    return false;
        }
    ...
    private Boolean mimicClaim(com.massivecraft.factions.FPlayer fplayer, com.massivecraft.factions.Faction faction, FLocation flocation) {
        ...
        boolean mustPay = Econ.shouldBeUsed() && !fplayer.isAdminBypassing() && !faction.isSafeZone() && !faction.isWarZone();
        double cost = 0.0;
        ...
        if (mustPay) {
            ...
            if (Conf.econClaimUnconnectedFee != 0.0 && faction.getLandRoundedInWorld(flocation.getWorldName()) > 0 && !Board.getInstance().isConnectedLocation(flocation, faction))
                cost += Conf.econClaimUnconnectedFee;

            if (Conf.bankEnabled && Conf.bankFactionPaysLandCosts && fplayer.hasFaction()) payee = faction;
            else payee = fplayer;
        ...
        // announce success
        Set<com.massivecraft.factions.FPlayer> informTheseFPlayers = new HashSet<>();
        ...
        if (Conf.logLandClaims)
            P.p.log(fplayer.getName() + " claimed land at (" + flocation.getCoordString() + ") for the faction: " + faction.getTag());
    ...
    private Chunk getFinalChunk(Set<Chunk> chunks) {
        ...
        return (Chunk) chunks.toArray()[chunks.size() - 1];
    }
